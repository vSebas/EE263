...
\sub{Unit balls:} \(B=\{x\mid \|x\|\le1\}\). In \(\R^2\), the
\(\ell_1\) ball is a diamond, the \(\ell_2\) ball is round, and the
\(\ell_\infty\) ball is a square.
...
Sum \(U+V=\{u+v\mid u\in U,\ v\in V\}\) and intersection \(U\cap V\)
are subspaces. \(U\cup V\) is a subspace iff \(U\subseteq V\) or
\(V\subseteq U\). Orthogonal
...
transformation in \(B\)-coordinates. Similar matrices have the same rank
and invertibility; \(B^{-1}AB=I\) implies \(A=I\).
...
for \(B\in\R^{m\times p}\), \(C\in\R^{p\times n}\),
\[
\begin{gathered}
...
\end{gathered}
\]
This is Sylvester's inequality. Invertible pre/post multiplication
preserves rank.
...
For any subspaces \(S,T\subseteq\R^n\),
\[
\begin{gathered}
...
\end{gathered}
\]
The decomposition \(x=s+t\) is unique iff \(S\cap T=\{0\}\), written
\(S\oplus T\).

\sub{Geometry preservation:} if \(U^TU=I\), then
\((Ua)^T(Ub)=a^Tb\), \(\|Ua\|=\|a\|\), and distances/angles are
preserved. Square orthogonal maps are rotations or reflections.
...
\(P=A(A^TA)^{-1}A^T\). An orthogonal projector satisfies
\(P=P^T=P^2\), \(\range(I-P)=\range(P)^\perp=\nullsp(P)\), and
\(x=Px+(I-P)x\) orthogonally. Hence \(\|Px\|\le\|x\|\), with equality iff
\(x\in\range(P)\), while \(Px=0\) iff \(x\in\range(P)^\perp\). If \(P,Q\)
are orthogonal projectors, \(PQ\) is an orthogonal projector iff
\(PQ=QP\); then it projects onto \(\range(P)\cap\range(Q)\).
...
\((I-QQ^T)x\). G-S depends on column order. Replacing
\((a_1,a_2,\ldots)\) by \((a_1,a_1+a_2,\ldots,a_1+\cdots+a_n)\) preserves
every leading span, so \(Q\) is unchanged up to signs, but \(R\) changes.
...
\sub{Independence:} columns independent iff $Ax=0\Rightarrow x=0$. Nonnegative independence: if every $u_i\ne0$ and $u_i^Tu_j\ge0$ for all $i,j$, then $\sum_i\alpha_i u_i=0$, $\alpha_i\ge0$ implies all $\alpha_i=0$ because norm squared is a sum of nonnegative terms.
...
\(A^TA\) is upper triangular, symmetry makes it diagonal, so columns are
orthogonal, not necessarily unit. Wide
...
\sub{Counterexamples:} \(S\cup T\) is a subspace only when one subspace
contains the other; two distinct lines through the origin disprove it.
