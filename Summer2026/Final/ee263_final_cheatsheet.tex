...
\(UU^T\ne I\) in general.
\[
\|U^Ty\|^2=\sum_i(u_i^Ty)^2\le\|y\|^2
\]
This is Bessel's inequality; equality holds iff \(y\in\range(U)\).
For square \(U\), equality for every \(y\) is Parseval's identity.

\sub{\(2\times2\) orthogonal maps:} \(\det U=+1\) means a rotation;
\(\det U=-1\) means a reflection. Orthogonal products and inverses are
orthogonal.

\sub{Orthogonal projection of a target:} let
\(S=\range(U)\), \(U^TU=I\). For target \(y\), the unique closest
point \(p\in S\) and perpendicular residual \(e\in S^\perp\) are
\[
\begin{gathered}
p=P_Sy=UU^Ty,\qquad e=y-p=(I-P_S)y,\\
U^Te=0,\qquad y=p+e,\qquad p\perp e
\end{gathered}
\]
\(P_S=P_S^T=P_S^2\). If \(A\) is any full-column-rank basis for \(S\),
\[
P_S=A(A^TA)^{-1}A^T=AA^\dagger
\]
Thus \(P_Sy=y\iff y\in S\), and \(P_Sy=0\iff y\in S^\perp\).
\[
\range(I-P_S)=\nullsp(P_S)=S^\perp
\]
\(\|P_Sy\|\le\|y\|\), with equality iff \(y\in S\). If orthogonal
projectors \(P,Q\) commute, \(PQ\) projects onto
\(\range(P)\cap\range(Q)\).

\sub{Projector recognition / complements:} \(P^2=P\) means projection
(possibly oblique); adding \(P^T=P\) makes it orthogonal. Orthogonal
projectors have eigenvalues \(0,1\), so
\[
\rank(P)=\tr(P),\qquad P_{S^\perp}=I-P_S,\qquad
P_SP_{S^\perp}=0
\]
For a unit direction \(q\),
\[
P_{\spn(q)}=qq^T,\quad P_{q^\perp}=I-qq^T,\quad
H=I-2qq^T
\]
\(H\) reflects through \(q^\perp\): the \(q\)-component changes sign and
all perpendicular components stay fixed.

\sub{Projection dictionary:} LS projects output target \(y\) onto
\(\range(A)\); least norm projects \(0\) onto the affine solution set
\(\{x:Ax=y\}\); PCA projects centered data onto a top-PC subspace. For
full-row-rank \(C\), projection of target \(x_0\) onto \(\{x:Cx=d\}\) is
\[
x^\star=x_0-C^T(CC^T)^{-1}(Cx_0-d)
\]
With metric \(\|z\|_W^2=z^TWz\), \(W\succ0\), replace this by
\[
x^\star=x_0-W^{-1}C^T(CW^{-1}C^T)^{-1}(Cx_0-d)
\]
For one hyperplane \(a^Tx=b\),
\[
x^\star=x_0-\frac{a^Tx_0-b}{\|a\|^2}a,\qquad
\operatorname{dist}(x_0,H)=\frac{|a^Tx_0-b|}{\|a\|}
\]
...
\]
For arbitrary rank, \(P_A=AA^\dagger\) and \(x=A^\dagger y\) selects the
minimum-norm coefficient vector producing the same closest fit.
...
\(W^{1/2}A,W^{1/2}y\).
The weighted fitted-output map is
\[
P_W=A(A^TWA)^{-1}A^TW
\]
\(P_W^2=P_W\), \(A^TW(P_Wy-y)=0\), and \(P_W^TW=WP_W\). It is
orthogonal in the \(W\)-inner product, though generally
\(P_W^T\ne P_W\) in the Euclidean one.
...
\]
\(x_{\rm ln}\) is therefore the orthogonal projection of the origin onto
the affine solution set \(\{x:Ax=y\}\).
...
The eigenspace is \(\nullsp(A-\lambda I)\). Its dimension (geometric
multiplicity) is at most the root multiplicity of \(\lambda\) in the
...
controls rotation/oscillation. The spectral radius is
\(\rho(A)=\max_i|\lambda_i|\); for diagonalizable \(A\),
\(A^k\to0\) iff \(\rho(A)<1\).
For real \(A\), a conjugate pair \(re^{\pm i\theta}\) produces a real
invariant plane spanned by the real and imaginary parts of its
eigenvector; on that plane \(A\) acts as scaling by \(r\) plus rotation.
...
\(w_i^*v_j=\delta_{ij}\).
For \(x=Vc=\sum_i c_iv_i\), \(c=V^{-1}x\), so
\[
Ax=\sum_i\lambda_i c_iv_i,\qquad c_i=w_i^*x
\]
The coefficients are not generally \(v_i^*x\); that simplification
requires an orthonormal eigenbasis.
Diagonalizability is equivalent to having enough eigenvectors across all
eigenspaces: the geometric multiplicities sum to \(n\).
...
\[
\begin{gathered}
\lambda_i(\alpha A+\beta I)=\alpha\lambda_i(A)+\beta,\\
\lambda_i(A^{-1})=\lambda_i(A)^{-1},\qquad
\lambda(A^T)=\lambda(A)
\end{gathered}
\]
The inverse rule requires \(A\) invertible. Similarity changes
coordinates, not eigenvalues.

\(AB\) and \(BA\) have the same nonzero eigenvalues:
\(ABv=\lambda v,\lambda\ne0\Rightarrow Bv\ne0\) and
\(BA(Bv)=\lambda Bv\).

\sub{Nonnormal caution:} for diagonalizable \(A=V\Lambda V^{-1}\),
\[
\|A^k\|\le\|V\|\,\|V^{-1}\|\,\max_i|\lambda_i|^k
\]
Nearly dependent eigenvectors make \(V\) ill-conditioned, so substantial
transient amplification can occur even when all \(|\lambda_i|<1\).
Symmetric matrices avoid this because \(V=Q\) is orthogonal.
...
\[
Ax=\sum_i\lambda_iq_i(q_i^Tx)
\]
Each \(q_iq_i^T\) is the one-dimensional orthogonal projector onto
\(\spn\{q_i\}\).
...
Indeed, with \(c=Q^Tx\),
\[
\frac{x^TAx}{x^Tx}=\frac{\sum_i\lambda_i c_i^2}{\sum_i c_i^2}
\]
is a weighted average of eigenvalues; the extrema are attained at
...
of \(F\) is \(\rank(A)\).
For a Gram matrix \(G=X^TX\), with \(x_i\) the columns of \(X\),
\[
\begin{gathered}
G_{ij}=x_i^Tx_j,\qquad z^TGz=\|Xz\|^2,\\
\nullsp(G)=\nullsp(X),\qquad\rank(G)=\rank(X)
\end{gathered}
\]
For any PSD \(A\), \(x^TAx=0\iff Ax=0\).
...
\]
\(S\succ0\) means all \(\lambda_i>0\), so \(S^{-1}\) exists and the set
is bounded. Singular \(S\succeq0\) instead gives an unbounded elliptical
cylinder along \(\nullsp(S)\). The longest-to-shortest semiaxis ratio is
\(\sqrt{\lambda_{\max}(S)/\lambda_{\min}(S)}\).
...
The left singular vectors of \(B\) give the ellipsoid axes and
\(\sigma_i(B)\) give their semiaxis lengths. Also
\[
\operatorname{vol}(\mathcal E)
=\operatorname{vol}(\mathbb B_n)|\det B|
=\frac{\operatorname{vol}(\mathbb B_n)}{\sqrt{\det S}}
\]
Under an invertible affine map \(y=Mx+d\), the center and quadratic
matrix transform as
\[
c_y=Mc+d,\qquad S_y=M^{-T}SM^{-1}
\]
...
\|A\|=\max_{\|u\|=\|v\|=1}|u^TAv|
\]
Consequently every entry, row norm, and column norm is at most \(\|A\|\):
\(|A_{ij}|\le\|A\|\), \(\|A^Te_i\|\le\|A\|\),
\(\|Ae_j\|\le\|A\|\).
...
\(\|A^{-1}\|=1/\sigma_{\min}(A)\).
Equality in the maximum/minimum gain bounds occurs at the corresponding
...
\sect{SVD (Lecture 17)}
\sub{Thin SVD:} every real matrix has an SVD. If
\(A\in\R^{m\times n}\) has rank \(r\),
\[
...
\]
Thus \(v_i\) is an input direction, \(u_i\) its output direction, and
\(\sigma_i\) its gain. The unit ball maps to an ellipsoid with semiaxes
\(\sigma_i u_i\); right-nullspace directions collapse to zero.
\[
\|Ax\|^2=\sum_{i=1}^r\sigma_i^2(v_i^Tx)^2
\]

\sub{How to construct / recognize it:}
\[
A^TAv_i=\sigma_i^2v_i,\qquad
u_i=\frac{Av_i}{\sigma_i}\quad(\sigma_i>0)
\]
Thus SVD comes from the symmetric EVD of \(A^TA\), but avoids squaring
the matrix numerically in actual computation. Orthogonal changes of
basis do not change singular values:
\(\sigma_i(Q_1AQ_2)=\sigma_i(A)\).
...
\[
\rank(A)=\#\{\sigma_i>0\},\quad
\|A\|=\sigma_1,\quad \|A\|_F^2=\sum_i\sigma_i^2
\]
For square \(A\), \(|\det A|=\prod_i\sigma_i\).
Tall full-column-rank means \(r=n\), every input direction has positive
gain, and \(A\) is one-to-one. Wide full-row-rank means \(r=m\), every
output is attainable, but \(n-m\) input directions lie in \(\nullsp(A)\).
...
\sub{EVD versus SVD:} EVD applies to square \(A\), can require complex
vectors, and can fail to diagonalize; it maps each eigenvector back onto
itself with signed/complex factor \(\lambda_i\). SVD always exists,
uses orthonormal input/output bases, and has nonnegative gains
\(\sigma_i\). For symmetric \(A\succeq0\), they coincide:
\(U=V=Q,\ \Sigma=\Lambda\).
...
\[
\begin{gathered}
\text{full column rank: }A^\dagger=(A^TA)^{-1}A^T,\\
\text{full row rank: }A^\dagger=A^T(AA^T)^{-1},\\
\text{invertible: }A^\dagger=A^{-1}
\end{gathered}
\]
...
\sub{Sensitivity / condition number:} on the nonzero singular spectrum,
\(\kappa=\sigma_1/\sigma_r\); rank deficiency gives
\(\kappa=\infty\). For invertible \(A\),
\[
\kappa(A)=\|A\|\|A^{-1}\|
...
to singularity. The bound is worst-case and is attained by aligned
singular directions; roughly \(\log_{10}\kappa\) decimal digits can be
lost.
...
Singular values below the modeling/noise scale are not reliably
distinguishable from zero; retaining only values above that floor gives
an effective rank and a simpler model.
...
Since centered columns sum to zero,
\(\rank(X_c)\le\min(n,N-1)\). Without centering, the first component can
mostly describe the mean rather than variation.
...
The coordinate \(u_j^T(x_i-\bar x)\) is the score of sample \(i\) on
component \(j\). Total variance is split among the PCs:
\[
...
In matrix form,
\[
Z=U_k^TX_c=\Sigma_kV_k^T,\qquad
\widehat X_c=U_kZ=U_kU_k^TX_c
\]
The score coordinates are centered and mutually uncorrelated:
\[
\frac1NZZ^T=\diag(\lambda_1,\ldots,\lambda_k)
\]
...
\]
Pythagoras gives, sample by sample,
\[
\|x_c\|^2=\|U_kU_k^Tx_c\|^2+\|(I-U_kU_k^T)x_c\|^2
\]
so maximizing captured variance and minimizing perpendicular
reconstruction error are the same problem. Equivalently,
\(\widehat X_c=U_k\Sigma_kV_k^T\) is the best rank-\(k\) approximation.
Reconstruction is exact once \(k\ge\rank(X_c)\).

\sub{Choosing / interpreting \(k\):} use a target \(\rho_k\), a scree-plot
elbow or spectral gap, and preferably validation for downstream error.
Standardize coordinates first when incompatible units/scales should have
equal importance; PCA itself is not scale invariant.
PC signs are arbitrary. With repeated eigenvalues, the principal
subspace is determined but an individual orthonormal basis inside it is
not.
...
coordinate \(z=S^{-1/2}(x-\bar x)\) has identity covariance, and
\[
\sqrt{(x-\bar x)^TS^{-1}(x-\bar x)}=\|z\|
\]
is Mahalanobis distance: distance from the mean measured in standard
deviations rather than raw units.
...
\sect{Projection--EVD--SVD--PCA Connections}
\sub{One recurring operation:} resolve into coordinates, weight selected
coordinates, then reconstitute:
\[
\begin{array}{c|c}
\text{orthogonal projection}&P_Sx=\sum_i(u_i^Tx)u_i\\
\text{symmetric EVD}&Ax=\sum_i\lambda_i(q_i^Tx)q_i\\
\text{SVD}&Ax=\sum_i\sigma_i(v_i^Tx)u_i\\
\text{PCA reconstruction}&\hat x=\bar x+\sum_{i=1}^k
u_iu_i^T(x-\bar x)
\end{array}
\]
A projector is a symmetric matrix whose EVD has only \(0\)'s and \(1\)'s.
Symmetric EVD permits signed weights \(\lambda_i\); SVD uses nonnegative
gains and different input/output directions. PCA applies symmetric EVD
to covariance, equivalently SVD to centered data, then keeps the
high-variance projectors.

\sub{Which target is closest?}
\[
\begin{aligned}
\text{LS output target }y &: AA^\dagger y\\
\text{least-norm target }0 &: A^\dagger y\\
\text{PCA centered target }x-\bar x &: U_kU_k^T(x-\bar x)
\end{aligned}
\]
Always identify the ambient space first: \(AA^\dagger\in\R^{m\times m}\)
projects outputs, while \(A^\dagger A\in\R^{n\times n}\) projects inputs.
...
\(\Prob(A\mid B)=\Prob(A)\).
For a disjoint exhaustive partition \(\{A_i\}\),
\[
\Prob(B)=\sum_i\Prob(B\mid A_i)\Prob(A_i)
\]
...
For a joint density \(f_{X,Y}\), marginalize via
\(f_X(x)=\int f_{X,Y}(x,y)\,dy\), and
\(f_{X\mid Y}(x\mid y)=f_{X,Y}(x,y)/f_Y(y)\).
...
For every deterministic \(a\),
\[
a^T\Sigma a=\Var(a^Tx)\ge0
\]
which proves \(\Sigma\succeq0\) and links covariance eigenvectors to PCA.
...
\(\Cov(Ax,By)=A\Sigma_{xy}B^T\).
If independent \(x,w\), then
\(\Cov(Ax+w)=A\Sigma_xA^T+\Sigma_w\).
Useful quadratic expectations:
\[
\E\|x\|^2=\tr\Sigma+\|\mu\|^2,\qquad
\E[x^TAx]=\tr(A\Sigma)+\mu^TA\mu
\]
for symmetric \(A\).
...
Standardizing gives \(Z=(X-\mu)/\sigma\sim\mathcal N(0,1)\).
...
\sub{Multivariate Gaussian:} for \(\Sigma\succ0\),
\[
f(x)=\frac{\exp\!\left(-\tfrac12
(x-\mu)^T\Sigma^{-1}(x-\mu)\right)}
{(2\pi)^{n/2}\sqrt{\det\Sigma}}
\]
Equal-density contours are covariance ellipsoids. If
\(z\sim\mathcal N(0,I)\), then
\(x=\mu+\Sigma^{1/2}z\sim\mathcal N(\mu,\Sigma)\).

\sect{Gaussian Conditioning / Practice Final}
\sub{MMSE meaning:} among all functions \(g(y)\),
\[
\hat x_{\rm MMSE}(y)=\E[x\mid y]
=\argmin_{g(y)}\E\!\left[\|x-g(y)\|^2\mid y\right]
\]
MAP instead maximizes the posterior density. Posterior mean and mode
coincide for a Gaussian.
...
Moreover,
\[
\E[\|x-\hat x_{\rm MMSE}\|^2\mid y]=\tr(\Cov(x\mid y))
\]
For arbitrary variables with finite second moments, the same mean formula
is the best affine estimator; it equals the full MMSE estimator when
\((x,y)\) is jointly Gaussian. Its error obeys the orthogonality principle
\[
\E[(x-\hat x)(y-\mu_y)^T]=0
\]
...
\]
Its posterior covariance is
\[
\Sigma_{x\mid y}
=\left(\tau^{-2}I+\sigma^{-2}A^TA\right)^{-1}
\]
Along right singular vector \(v_i\), the posterior variance is
\[
\Var(v_i^Tx\mid y)=
\frac{\tau^2\sigma^2}{\sigma^2+\tau^2\sigma_i^2}
\]
Measurements reduce uncertainty most in high-gain singular directions.
...
More generally, Gaussian \(x\sim\mathcal N(\mu_x,\Sigma_x)\) and
\(w\sim\mathcal N(0,\Sigma_w)\) give both MAP and MMSE as the minimizer of
\[
(y-Ax)^T\Sigma_w^{-1}(y-Ax)
...
\sect{Decomposition and Model Dictionaries}
\sub{Which decomposition?}
\(\textbf{QR}\) produces an orthonormal basis for \(\range(A)\) plus
triangular coordinates; use it for stable solves and LS.
\(\textbf{EVD}\) exposes invariant modes of a square map and simplifies
\(A^k,f(A)\), but a full eigenbasis may not exist.
\(\textbf{Symmetric EVD}\) always has an orthonormal basis; use it for
quadratic forms, PSD tests, covariance, and ellipsoid axes.
\(\textbf{SVD}\) works for every rectangular matrix and exposes gains,
rank, four spaces, pseudoinverse, conditioning, and low-rank structure.
\(\textbf{PCA}\) is not a separate factorization: center the data, then
use the symmetric EVD of covariance or SVD of the centered matrix.

\sub{Singular-direction filter dictionary:} let \(s_i>0\) be a singular
value and \(d_i=u_i^Ty\). The coefficient on \(v_i\) is
\[
\begin{array}{c|c}
\text{method}&v_i\text{ coefficient}\\ \hline
\text{pseudoinverse}&d_i/s_i\\
\text{TSVD at }k&d_i/s_i\ (i\le k),\ 0\ (i>k)\\
\text{Tikhonov}&s_i d_i/(s_i^2+\mu)\\
\text{Gaussian MMSE}&\tau^2s_i d_i/(\tau^2s_i^2+\sigma_w^2)
\end{array}
\]
Relative to direct inversion, the last two shrink by
\[
\frac{s_i^2}{s_i^2+\mu},\qquad
\frac{\tau^2s_i^2}{\tau^2s_i^2+\sigma_w^2}
\]
Small-\(s_i\) modes are precisely where inversion amplifies noise and
regularization suppresses it.

\sub{Ellipsoid dictionary:}
\[
\begin{gathered}
\mathcal E=\{c+Bz:\|z\|\le1\}
=\{x:(x-c)^TS(x-c)\le1\},\\
S=(BB^T)^{-1}
\end{gathered}
\]
Axes are the left singular vectors of \(B\), with lengths
\(\sigma_i(B)\). For the covariance ellipsoid
\((x-c)^T\Sigma^{-1}(x-c)\le\alpha\), one may take
\[
B=\sqrt\alpha\,\Sigma^{1/2},\qquad
S=\alpha^{-1}\Sigma^{-1}
\]
Thus PCA eigenvectors give the axes, covariance eigenvalues give squared
standard deviations, and Mahalanobis distance measures radius in these
standardized coordinates.

\sub{Four-space projector dictionary:} for thin
\(A=U_r\Sigma_rV_r^T\),
\[
\begin{array}{c|c}
\range(A)&AA^\dagger=U_rU_r^T\\
\nullsp(A^T)&I-AA^\dagger=I-U_rU_r^T\\
\range(A^T)&A^\dagger A=V_rV_r^T\\
\nullsp(A)&I-A^\dagger A=I-V_rV_r^T
\end{array}
\]
Hence
\[
\min_x\|Ax-y\|=\|(I-AA^\dagger)y\|,\qquad
\hat y=AA^\dagger y
\]
Also
\[
(A^\dagger)^T=(A^T)^\dagger,\qquad
\|A^\dagger\|=\frac1{\sigma_r}
\]
where \(\sigma_r\) is the smallest positive singular value.

\sub{Closest-point proof:} verify \(p\in S\) and
\(e=y-p\in S^\perp\). For every \(z\in S\), Pythagoras gives
\[
\|y-z\|^2=\|e+(p-z)\|^2
=\|e\|^2+\|p-z\|^2\ge\|e\|^2
\]
so \(p\) is uniquely closest. For an affine set,
\(P_{x_0+S}(y)=x_0+P_S(y-x_0)\).

\sub{Singular-value perturbation:}
\[
|\sigma_i(A+E)-\sigma_i(A)|\le\|E\|
\]

\sub{Normal-equation conditioning:}
\[
\lambda_i(A^TA)=\sigma_i(A)^2,\qquad
\kappa(A^TA)=\kappa(A)^2
\]
for full-column-rank \(A\). Normal equations square conditioning and can
lose about twice as many digits; prefer QR or SVD numerically.
